...
def OPUS_read_abs():
    ...
    cmd = add_modbus_crc(b'\x01\x03\x0A\x72\x00\x7C')  # wv: 063-124
    trios_answer, outnum = modbus_send_cmd(cmd)
    outputABS063_124 = read_Float(trios_answer[0:outnum[2]], 62)
    # print(outputABS063_124)

    cmd = add_modbus_crc(b'\x01\x03\x0A\xB0\x00\x7C')  # wv: 125-186
    trios_answer, outnum = modbus_send_cmd(cmd)
    outputABS125_186 = read_Float(trios_answer[0:outnum[2]], 62)
    # print(outputABS125_186)

    cmd = add_modbus_crc(b'\x01\x03\x0A\xEE\x00\x7C')  # wv: 187-248 09 A8
    trios_answer, outnum = modbus_send_cmd(cmd)
...
if __name__ == "__main__":
    ...
    portstring = conf.get('OPUS', 'COM-Port', fallback='/dev/tty.usbserial-AU04OTVX')
